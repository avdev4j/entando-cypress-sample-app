...
import { apiBarsGet, apiBarsDelete } from 'api/bars';
import { reducer, initialState } from 'state/bar.reducer';
import { ADD_FILTER, UPDATE_FILTER, DELETE_FILTER, CLEAR_FILTERS } from 'state/filter.types';
import { DELETE, ERROR_FETCH, CLEAR_ERRORS, READ_ALL, CLEAR_ITEMS } from 'state/bar.types';
...
class BarTableContainer extends Component {
  constructor(props) {
    super(props);
  ...
  async fetchData() {
    ...
    if (authenticated) {
      try {
        ...
        const { bars, headers } = await apiBarsGet(serviceUrl, requestParameters);
        const barCount = (headers && headers['X-Total-Count']) || 0;

        this.dispatch({
          type: READ_ALL,
          payload: {
            items: paginationMode === 'infinite-scroll' ? [...items, ...bars] : bars,
            count: barCount,
          },
        });
      } catch (err) {
  ...
  async handleDelete(item) {
    ...
    if (authenticated) {
      try {
        await apiBarsDelete(serviceUrl, item.id);
        onDelete(item);
    ...
    const Actions = ({ item }) =>
      onDelete ? (
        <ConfirmationDialogTrigger
          ...
          dialog={{
            title: t('entities.bar.deleteDialog.title'),
            description: t('entities.bar.deleteDialog.description'),
        ...
        <AuthenticatedView keycloak={keycloak}>
          ...
          <PaginationWrapper items={items} paginationMode={paginationMode} count={count}>
            <div className={classes.tableWrapper}>
              <BarTable items={items} onSelect={onSelect} Actions={Actions} />
            </div>
          </PaginationWrapper>
        </AuthenticatedView>
        <Notification
...
BarTableContainer.defaultProps = {
  onAdd: () => {},
  onDelete: () => {},
  onError: () => {},
  onSelect: () => {},
...
export default withKeycloak(
  withStyles(styles)(
    withTranslation(undefined, { withRef: true })(withPagination(BarTableContainer))
  )
);
