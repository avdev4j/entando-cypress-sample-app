...
1. Go to widget source (<MICROSERVICE_FOLDER>/ui/widgets/bar/formWidget/>):
    1. Run `npm i`
...
-   **bar.form.createError** (fired when an error occurs during bar creation)
-   **bar.form.create** (fired when the bar is successfully created)
-   **bar.form.updateError** (fired when an error occurs during bar update)
-   **bar.form.update** (fired when the bar is successfully updated)
