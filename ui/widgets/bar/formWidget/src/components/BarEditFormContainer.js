...
import { apiBarGet, apiBarDelete, apiBarPut } from 'api/bars';
import Notification from 'components/common/Notification';
import BarForm from 'components/BarForm';

class BarEditFormContainer extends PureComponent {
  constructor(props) {
    super(props);
  ...
  async handleSubmit(bar) {
    ...
    if (authenticated) {
      try {
        const updatedBar = await apiBarPut(serviceUrl, bar);
        onUpdate(updatedBar);
  ...
  async handleDelete(bar) {
    const { t, onDelete, keycloak, serviceUrl } = this.props;
    const authenticated = keycloak.initialized && keycloak.authenticated;

    if (authenticated) {
      try {
        await apiBarDelete(serviceUrl, bar.id);
        onDelete(bar);
        this.setState({
          bar: null,
  ...
  render() {
    ...
    const { notificationMessage, notificationStatus, bar } = this.state;

    let form;
    if (typeof bar === 'undefined') {
      form = t('entities.bar.notFound');
    } else if (bar === null) {
      form = t('entities.bar.deleted');
    } else {
      form = (
        <BarForm
          bar={bar}
...
BarEditFormContainer.propTypes = {
...
BarEditFormContainer.defaultProps = {
  onCancelEditing: () => {},
  onDelete: () => {},
  onUpdate: () => {},
  onError: () => {},
...
export default withKeycloak(withTranslation()(BarEditFormContainer));
